...
figures_folder = ei.out_folder+'figures/'

if not os.path.exists(figures_folder):
    create_folder = ask_yn('Create folder <%s>? ' % figures_folder)
    if (create_folder):
        #print 'Creating folder <%s>' % figures_folder
        os.makedirs(figures_folder)        
else:
    print 'Folder <%s> already exists.' % figures_folder
...
if (start_exps>0):
    log_filename = params_folder + "output.log"
...
#g_idx = exp_params_names.index('pf_generations')
#p_idx = exp_params_names.index('pf_particles')
    ...
    for r in range(ei.start_rep,ei.stop_rep+1):
        ...
        if (os.path.isfile(filename)):
            ...
            if (start_exps>0):
                ...
                subprocess.call(full_command, shell=True, stdout=log_file)       
                log_file.close()
